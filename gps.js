...
// Bug #6 fix: Load settings dari server ATAU localStorage
// Server settings akan di-load async saat initGPS dipanggil
...
// Bug #6 fix: Sinkronisasi GPS settings dari server
async function syncGpsSettingsFromServer() {
    try {
        const result = await apiGetSettings();
        if (result.success && result.settings) {
            const s = result.settings;
            // Hanya update jika ada data GPS di server
            if (s.gpsLatitude || s.gpsLongitude) {
                const serverSettings = {
                    enableRadiusCheck: s.gpsEnableRadius === 'true' || s.gpsEnableRadius === true,
                    latitude: parseFloat(s.gpsLatitude) || -8.5833,
                    longitude: parseFloat(s.gpsLongitude) || 116.1167,
                    maxRadius: parseInt(s.gpsMaxRadius) || 500
                };

                // Update localStorage dengan data server
                localStorage.setItem('gpsSettings', JSON.stringify(serverSettings));

                // Update SCHOOL_CONFIG
                SCHOOL_CONFIG.enableRadiusCheck = serverSettings.enableRadiusCheck;
                SCHOOL_CONFIG.latitude = serverSettings.latitude;
                SCHOOL_CONFIG.longitude = serverSettings.longitude;
                SCHOOL_CONFIG.maxRadius = serverSettings.maxRadius;

                console.log('GPS settings synced from server:', serverSettings);
            }
        }
    } catch (e) {
        console.error('Error syncing GPS settings from server:', e);
        // Gunakan localStorage sebagai fallback (sudah di-load di getSchoolConfig)
    }
}
...
function initGPS() {
    ...
    // Bug #6 fix: Sync GPS settings dari server
    syncGpsSettingsFromServer();
